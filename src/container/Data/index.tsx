...
const Data = () => {
  ...
  const [expenseData, setExpenseData] = useState([] as TotalItem[]); // 支出数据
  const [incomeData, setIncomeData] = useState([] as TotalItem[]); // 收入数据
  ...
  const getData = async () => {
    ...
    // 过滤支出和收入
    const expense_data = data.total_data.filter((item: TotalItem) => item.pay_type == 1).sort((a, b) => b.number - a.number); // 过滤出账单类型为支出的项
    const income_data = data.total_data.filter((item: TotalItem) => item.pay_type == 2).sort((a, b) => b.number - a.number); // 过滤出账单类型为收入的项
