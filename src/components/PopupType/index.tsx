...
const PopupType = forwardRef(({ onSelect }: PropsType, ref) => {
  ...
  useEffect(() => {
    const getTypeList = async () => {
      ...
      setExpense(list.filter((i: SelectType) => i.type == 1))
      setIncome(list.filter((i: SelectType) => i.type == 2))
    }
    getTypeList()
  }, [])
  ...
  // 选择类型回调
  const choseType = (item: SelectType) => {
    setActive(item.id)
    setShow(false)
  ...
  >
    <div className={s.popupType}>
      ...
      <div className={s.content}>
        <div onClick={() => choseType({ id: 'all' } as SelectType)} className={cx({ [s.all]: true, [s.active]: active == 'all' })}>全部类型</div>
